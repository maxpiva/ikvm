    ...
    partial class RuntimeByteCodeJavaType
    {
        ...
        private sealed partial class JavaTypeImpl : DynamicImpl
        {
            ...
            bool SupportsCallerID(Method method)
            {
                ...
                else if ((classFile.IsFinal || classFile.Name == "java.lang.Runtime" || classFile.Name == "java.io.ObjectStreamClass") && wrapper.BaseTypeWrapper.GetMethod(method.Name, method.Signature, true) == null && !HasInterfaceMethod(wrapper, method.Name, method.Signature))
                {
            ...
            static bool HasInterfaceMethod(RuntimeJavaType tw, string name, string signature)
            {
                for (; tw != null; tw = tw.BaseTypeWrapper)
                {
                    ...
                    {
                        if (iface.GetMethod(name, signature, false) != null)
                        {
                            return true;
                        }
            ...
            internal void CreateStep2()
            {
                ...
                try
                {
                    ...
                    if (classFile.RuntimeVisibleTypeAnnotations.Count > 0)
                        wrapper.Context.AttributeHelper.SetRuntimeVisibleTypeAnnotationsAttribute(typeBuilder, in classFile.RuntimeVisibleTypeAnnotations);
            ...
            private RuntimeJavaMethod GetMethodWrapperDuringCtor(RuntimeJavaType lookup, IList<RuntimeJavaMethod> methods, string name, string sig)
            {
                if (lookup == wrapper)
                {
                    ...
                    {
                        return lookup.BaseTypeWrapper.GetMethod(name, sig, true);
                    }
                }
                else
                {
                    return lookup.GetMethod(name, sig, true);
                }
            }
            ...
            void AddMirandaMethods(List<RuntimeJavaMethod> methods, List<RuntimeJavaMethod[]> baseMethods, RuntimeJavaType tw)
            {
                foreach (var iface in tw.Interfaces)
                {
                    ...
                    if (iface.IsPublic && wrapper.IsInterface)
                        continue;
                    ...
                    foreach (var ifmethod in iface.GetMethods())
                    {
                        ...
                        if (ifmethod.IsVirtual)
                        {
                            ...
                            while (lookup != null)
                            {
                                ...
                                if (mw.IsStatic == false || mw.DeclaringType == wrapper)
                                    break;
            ...
            void AddDelegateInvokeStubs(RuntimeJavaType tw, ref RuntimeJavaMethod[] methods)
            {
                foreach (var iface in tw.Interfaces)
                {
                    ...
                    {
                        var mw = new DelegateInvokeStubMethodWrapper(wrapper, iface.DeclaringTypeWrapper.TypeAsBaseType, iface.GetMethods()[0].Signature);
            ...
            {
                if (fw is RuntimeByteCodePropertyJavaField propertyField)
                {
                    propertyField.DoLink(typeBuilder);
                    return null;
                }
                ...
                if (wrapper.ClassLoader.RemoveUnusedFields &&
                    ...
                    fw.IsStatic &&
                    fw.IsFinal &&
                    fw.IsSerialVersionUID == false &&
                    classFile.Fields[fieldIndex].Annotations == null &&
                    classFile.IsReferenced(classFile.Fields[fieldIndex]) == false)
                {
                ...
                if (ReferenceEquals(realFieldName, fld.Name) == false)
...
#if IMPORTER
                {
                    ...
                    if (fld.RuntimeVisibleTypeAnnotations.Count > 0)
                        wrapper.Context.AttributeHelper.SetRuntimeVisibleTypeAnnotationsAttribute(field, in fld.RuntimeVisibleTypeAnnotations);
                }
#endif
            ...
            internal override DynamicImpl Finish()
            {
                ...
                foreach (var iface in wrapper.interfaces)
                {
                    iface.Finish();
                    iface.LinkAll();
                }
            ...
            FinishedTypeImpl FinishCore()
            {
                ...
                try
                {
                    ...
                    if (innerclasses != null)
                    {
                        ...
                        {
                            if (tw is RuntimeByteCodeJavaType dtw)
                            {
            ...
            bool IsValidAnnotationElementType(string type)
            {
                if (type[0] == '[')
            ...
            sealed class AnnotationBuilder : Annotation
            {
                ...
                internal void Link()
                {
                    ...
                    var name = UnicodeUtil.EscapeInvalidSurrogates(o.classFile.Name);
                ...
                static Type TypeWrapperToAnnotationParameterType(RuntimeJavaType tw)
                {
                    var isArray = false;
                    if (tw.IsArray)
                    {
                ...
                static void EmitSetValueCall(RuntimeJavaType annotationAttributeBaseType, CodeEmitter ilgen, string name, RuntimeJavaType tw, int argIndex)
                {
                    ...
                    var setValueMethod = annotationAttributeBaseType.GetMethod("setValue", "(Ljava.lang.String;Ljava.lang.Object;)V", false);
                    setValueMethod.Link();
                ...
                internal void Finish(JavaTypeImpl o)
                {
                    ...
                    if (!unsupported)
                    {
                        if (requiredArgCount > 0)
                        {
                            ...
                            var reqArgConstructor = ReflectUtil.DefineConstructor(attributeTypeBuilder, MethodAttributes.Public, args);
                    ...
                    annotationAttributeBaseType.GetMethod("<init>", "(Ljava.lang.Class;)V", false).EmitCall(ilgen);
                    ilgen.Emit(OpCodes.Ret);
                    ilgen.DoEmit();
                    ...
                    annotationAttributeBaseType.GetMethod("setDefinition", "([Ljava.lang.Object;)V", false).EmitCall(ilgen);
                    ilgen.Emit(OpCodes.Ret);
                    ilgen.DoEmit();

                    var getValueMethod = annotationAttributeBaseType.GetMethod("getValue", "(Ljava.lang.String;)Ljava.lang.Object;", false);
                    var getByteValueMethod = annotationAttributeBaseType.GetMethod("getByteValue", "(Ljava.lang.String;)B", false);
                    var getBooleanValueMethod = annotationAttributeBaseType.GetMethod("getBooleanValue", "(Ljava.lang.String;)Z", false);
                    var getCharValueMethod = annotationAttributeBaseType.GetMethod("getCharValue", "(Ljava.lang.String;)C", false);
                    var getShortValueMethod = annotationAttributeBaseType.GetMethod("getShortValue", "(Ljava.lang.String;)S", false);
                    var getIntValueMethod = annotationAttributeBaseType.GetMethod("getIntValue", "(Ljava.lang.String;)I", false);
                    var getFloatValueMethod = annotationAttributeBaseType.GetMethod("getFloatValue", "(Ljava.lang.String;)F", false);
                    var getLongValueMethod = annotationAttributeBaseType.GetMethod("getLongValue", "(Ljava.lang.String;)J", false);
                    var getDoubleValueMethod = annotationAttributeBaseType.GetMethod("getDoubleValue", "(Ljava.lang.String;)D", false);
                    ...
                    for (int i = 0; i < o.methods.Length; i++)
                    {
                        ...
                        if (o.methods[i].IsVirtual)
                        {
                                ...
                                !(o.methods[i].Name == "value" && requiredArgCount == 0))
                            {
                                ...
                                if (argType != null)
                                {
                                    var property = attributeTypeBuilder.DefineProperty(o.methods[i].Name, PropertyAttributes.None, argType, Type.EmptyTypes);
                                    context.AttributeHelper.HideFromJava(property);

                                    var setter = attributeTypeBuilder.DefineMethod("set_" + o.methods[i].Name, MethodAttributes.Public, context.Types.Void, [argType]);
                                    ...
                                    var getter = attributeTypeBuilder.DefineMethod("get_" + o.methods[i].Name, MethodAttributes.Public, argType, Type.EmptyTypes);
                ...
                CustomAttributeBuilder MakeCustomAttributeBuilder(RuntimeClassLoader loader, object annotation)
                {
                    ...
                    ConstructorInfo ctor = defineConstructor != null
                        ...
                        : context.Resolver.ResolveRuntimeType("IKVM.Attributes.DynamicAnnotationAttribute").AsReflection().GetConstructor([context.Types.Object.MakeArrayType()]);

                    return new CustomAttributeBuilder(ctor, [AnnotationDefaultAttribute.Escape(QualifyClassNames(loader, annotation))]);
                }
            ...
            internal override RuntimeJavaType[] InnerClasses => throw new InvalidOperationException("InnerClasses is only available for finished types");

            internal override RuntimeJavaType DeclaringTypeWrapper => throw new InvalidOperationException("DeclaringTypeWrapper is only available for finished types");
            ...
            RuntimeJavaMethod[] FindBaseMethods7(string name, string sig, bool packageFinal, out bool explicitOverride)
            {
                ...
                while (tw != null)
                {
                    var baseMethod = tw.GetMethod(name, sig, true);
                    if (baseMethod == null)
                        break;
                ...
                while (tw != null)
                {
                    ...
                    else if (topPublicOrProtectedMethod == null)
                    {
                        if (explicitOverride)
                        {
                            ...
                            while (tw != null)
                            {
                                var baseMethod2 = tw.GetMethod(name, sig, true);
                    ...
                    else
                    {
                            ...
                            || (majorVersion == 0 && (LinkAndGetMethod(baseMethod).Attributes & MethodAttributes.MemberAccessMask) == MethodAttributes.Assembly)))
                        {
                            ...
                            return [baseMethod, topPublicOrProtectedMethod];
                        }
                ...
                while (tw != null)
                {
                    ...
                    else if (baseMethod.IsPublic || baseMethod.IsProtected)
                    {
                        ...
                        if (!baseMethod.DeclaringType.IsPackageAccessibleFrom(wrapper))
                        {
                            ...
                            while (tw != null)
                            {
                                RuntimeJavaMethod baseMethod2 = tw.GetMethod(name, sig, true);
                                if (baseMethod2 == null)
                                {
                                    break;
                                }
            ...
            static MethodInfo GetBaseFinalizeMethod(RuntimeJavaType wrapper)
            {
                for (; ; )
                {
                    ...
                    var mw = dtw.GetMethod(StringConstants.FINALIZE, StringConstants.SIG_VOID, false);
                    if (mw != null)
                        mw.Link();
                ...
                try
                {
                    ...
                    if (index >= classFile.Methods.Length)
                    {
                        ...
                        if (methods[index].IsMirandaMethod)
                        {
                            ...
                            if (wrapper.BaseTypeWrapper == null || (baseMiranda = wrapper.BaseTypeWrapper.GetMethod(mw.Name, mw.Signature, true)) == null || !baseMiranda.IsMirandaMethod)
                            {
                    ...
                    if (m.RuntimeVisibleTypeAnnotations.Count > 0)
                        wrapper.Context.AttributeHelper.SetRuntimeVisibleTypeAnnotationsAttribute(method, in m.RuntimeVisibleTypeAnnotations);
            ...
            MethodBuilder GenerateMethod(int index, Method m, ref bool setModifiers)
            {
                ...
                if (mb == null)
                {
                    ...
                    if (needFinalize)
                    {
                        ...
                        var mwClash = wrapper.GetMethod(finalizeName, StringConstants.SIG_VOID, true);
                    ...
                    {
                        var cab = new CustomAttributeBuilder(wrapper.Context.Resolver.ResolveRuntimeType("IKVM.Attributes.AnnotationDefaultAttribute").AsReflection().GetConstructor([wrapper.Context.Types.Object]), [AnnotationDefaultAttribute.Escape(classFile.Methods[index].AnnotationDefault)]);
